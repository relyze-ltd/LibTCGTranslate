...
static inline void cpu_get_tb_cpu_state(CPUX86State *env, target_ulong *pc, target_ulong *cs_base, uint32_t *flags)
{
	*cs_base = env->segs[R_CS].base;
	*pc = *cs_base + env->eip;
	*flags = env->hflags | (env->eflags & (IOPL_MASK | TF_MASK | RF_MASK | VM_MASK | AC_MASK));
}

#if defined(TARGET_X86) || defined(TARGET_X86_64)
static void i386_cpu_init(CPUX86State *env)
{
	env->hflags |= HF_CS32_MASK | HF_SS32_MASK | HF_CS64_MASK;
	env->hflags |= HF_PE_MASK | HF_CPL_MASK;
	env->hflags |= HF_OSFXSR_MASK;

	env->features[FEAT_1_EDX] = CPUID_VME | CPUID_SSE2 | CPUID_SSE | CPUID_FXSR | CPUID_MMX |
		CPUID_CLFLUSH | CPUID_PSE36 | CPUID_PAT | CPUID_CMOV | CPUID_MCA |
		CPUID_PGE | CPUID_MTRR | CPUID_SEP | CPUID_APIC | CPUID_CX8 |
		CPUID_MCE | CPUID_PAE | CPUID_MSR | CPUID_TSC | CPUID_PSE |
		CPUID_DE | CPUID_FP87;

	env->features[FEAT_1_ECX] = CPUID_EXT_AVX | CPUID_EXT_XSAVE | CPUID_EXT_AES |
		CPUID_EXT_TSC_DEADLINE_TIMER | CPUID_EXT_POPCNT |
		CPUID_EXT_X2APIC | CPUID_EXT_SSE42 | CPUID_EXT_SSE41 |
		CPUID_EXT_CX16 | CPUID_EXT_SSSE3 | CPUID_EXT_PCLMULQDQ |
		CPUID_EXT_SSE3;

	env->features[FEAT_7_0_EBX] = 0;

	env->features[FEAT_7_0_ECX] = 0;

	env->features[FEAT_7_0_EDX] = 0;

	env->features[FEAT_8000_0001_EDX] = CPUID_EXT2_LM | CPUID_EXT2_RDTSCP | CPUID_EXT2_NX |
		CPUID_EXT2_SYSCALL;

	env->features[FEAT_8000_0001_ECX] = CPUID_EXT3_LAHF_LM;

	env->features[FEAT_8000_0007_EDX] = 0;

	env->features[FEAT_C000_0001_EDX] = 0;

	env->features[FEAT_KVM] = 0;

	env->features[FEAT_HYPERV_EAX] = 0;

	env->features[FEAT_HYPERV_EBX] = 0;

	env->features[FEAT_HYPERV_EDX] = 0;

	env->features[FEAT_SVM] = 0;

	env->features[FEAT_XSAVE] = CPUID_XSAVE_XSAVEOPT;

	env->features[FEAT_6_EAX] = CPUID_6_EAX_ARAT;

	env->features[FEAT_XSAVE_COMP_LO] = 0;

	env->features[FEAT_XSAVE_COMP_HI] = 0;

	env->features[FEATURE_WORDS] = 0;
}
#endif
