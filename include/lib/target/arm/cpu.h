 ...
 */
typedef void ARMELChangeHook(struct ARMCPU *cpu, void *opaque);
...
//int arm_cpu_write_elf64_note(WriteCoreDumpFunction f, CPUState *cs,int cpuid, void *opaque);
//int arm_cpu_write_elf32_note(WriteCoreDumpFunction f, CPUState *cs,int cpuid, void *opaque);
...
{
    return (el << 2) | (unsigned int)handler;
}
 ...
 */
static uint32_t arm_regime_tbi0(CPUARMState *env, ARMMMUIdx mmu_idx)
{
	return 0;
}
/**
 ...
 */
static uint32_t arm_regime_tbi1(CPUARMState *env, ARMMMUIdx mmu_idx)
{
	return 0;
}
#else
...
static uint32_t arm_ldl_code(void *env, target_ulong addr, bool sctlr_b)
{
	uint32_t buffer = cpu_ldul_code(env, addr);

	if (bswap_code(sctlr_b))
		return (((buffer & 0x00FF0000) >> 8) | ((buffer & 0xFF000000) >> 24) | ((buffer & 0x000000FF) << 24) | ((buffer & 0x0000FF00) << 8));

	return buffer;
}

static uint16_t arm_lduw_code(void *env, target_ulong addr, bool sctlr_b)
{
	uint16_t buffer = cpu_lduw_code(env, addr);
...
static void arm_cpu_init2(CPUARMState *env)
{
	if (FEATURE_HAS(env, ARM_FEATURE_V8)) {

		FEATURE_ENABLE(env, ARM_FEATURE_V7);

		FEATURE_ENABLE(env, ARM_FEATURE_ARM_DIV);

		FEATURE_ENABLE(env, ARM_FEATURE_LPAE);
	}

	if (FEATURE_HAS(env, ARM_FEATURE_V7)) {

		FEATURE_ENABLE(env, ARM_FEATURE_VAPA);

		FEATURE_ENABLE(env, ARM_FEATURE_THUMB2);

		FEATURE_ENABLE(env, ARM_FEATURE_MPIDR);

		if (!FEATURE_HAS(env, ARM_FEATURE_M))
			FEATURE_ENABLE(env, ARM_FEATURE_V6K);
		else
			FEATURE_ENABLE(env, ARM_FEATURE_V6);

		FEATURE_ENABLE(env, ARM_FEATURE_VBAR);
	}

	if (FEATURE_HAS(env, ARM_FEATURE_V6K)) {

		FEATURE_ENABLE(env, ARM_FEATURE_V6);

		FEATURE_ENABLE(env, ARM_FEATURE_MVFR);
	}

	if (FEATURE_HAS(env, ARM_FEATURE_V6)) {

		FEATURE_ENABLE(env, ARM_FEATURE_V5);

		if (!FEATURE_HAS(env, ARM_FEATURE_M))
			FEATURE_ENABLE(env, ARM_FEATURE_AUXCR);
	}

	if (FEATURE_HAS(env, ARM_FEATURE_V5))
		FEATURE_ENABLE(env, ARM_FEATURE_V4T);

	if (FEATURE_HAS(env, ARM_FEATURE_M))
		FEATURE_ENABLE(env, ARM_FEATURE_THUMB_DIV);

	if (FEATURE_HAS(env, ARM_FEATURE_ARM_DIV))
		FEATURE_ENABLE(env, ARM_FEATURE_THUMB_DIV);

	if (FEATURE_HAS(env, ARM_FEATURE_VFP4))
	{
		FEATURE_ENABLE(env, ARM_FEATURE_VFP3);
		FEATURE_ENABLE(env, ARM_FEATURE_VFP_FP16);
	}

	if (FEATURE_HAS(env, ARM_FEATURE_VFP3))
		FEATURE_ENABLE(env, ARM_FEATURE_VFP);

	if (FEATURE_HAS(env, ARM_FEATURE_LPAE))
	{
		FEATURE_ENABLE(env, ARM_FEATURE_V7MP);
		FEATURE_ENABLE(env, ARM_FEATURE_PXN);
	}

	if (FEATURE_HAS(env, ARM_FEATURE_CBAR_RO))
		FEATURE_ENABLE(env, ARM_FEATURE_CBAR);

	if (FEATURE_HAS(env, ARM_FEATURE_THUMB2) && !FEATURE_HAS(env, ARM_FEATURE_M))
		FEATURE_ENABLE(env, ARM_FEATURE_THUMB_DSP);

	if (FEATURE_HAS(env, ARM_FEATURE_EL3))
		FEATURE_ENABLE(env, ARM_FEATURE_VBAR);
}
#endif

#if defined(TARGET_ARM)
static void arm_cpu_init(CPUARMState *env)
{
	FEATURE_ENABLE(env, ARM_FEATURE_V7);

	FEATURE_ENABLE(env, ARM_FEATURE_VFP4);

	FEATURE_ENABLE(env, ARM_FEATURE_NEON);

	FEATURE_ENABLE(env, ARM_FEATURE_THUMB2EE);

	FEATURE_ENABLE(env, ARM_FEATURE_ARM_DIV);

	FEATURE_ENABLE(env, ARM_FEATURE_GENERIC_TIMER);

	FEATURE_ENABLE(env, ARM_FEATURE_DUMMY_C15_REGS);

	FEATURE_ENABLE(env, ARM_FEATURE_CBAR_RO);

	FEATURE_ENABLE(env, ARM_FEATURE_LPAE);

	FEATURE_ENABLE(env, ARM_FEATURE_EL3);

	arm_cpu_init2(env);
}

#elif defined(TARGET_AARCH64)
static void a64_cpu_init(CPUARMState *env)
{
	env->aarch64 = true;

	FEATURE_ENABLE(env, ARM_FEATURE_V8);
	...
	FEATURE_ENABLE(env, ARM_FEATURE_AARCH64);

	FEATURE_ENABLE(env, ARM_FEATURE_CBAR_RO);

	FEATURE_ENABLE(env, ARM_FEATURE_V8_AES);

	FEATURE_ENABLE(env, ARM_FEATURE_V8_SHA1);

	FEATURE_ENABLE(env, ARM_FEATURE_V8_SHA256);

	FEATURE_ENABLE(env, ARM_FEATURE_V8_PMULL);

	FEATURE_ENABLE(env, ARM_FEATURE_CRC);

	FEATURE_ENABLE(env, ARM_FEATURE_EL2);

	FEATURE_ENABLE(env, ARM_FEATURE_EL3);

	FEATURE_ENABLE(env, ARM_FEATURE_PMU);
